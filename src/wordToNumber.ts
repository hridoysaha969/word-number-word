...
export function wordToNumber(words: string): number {
  // Handle negatives
  const isNegative = words.startsWith("minus ");
  if (isNegative) words = words.slice(6);

  // Handle decimals
  if (words.includes("point")) {
    const [integerPart, decimalPart] = words.split(" point ");
    return (
      parseFloat(`${wordToNumber(integerPart)}.${wordToNumber(decimalPart)}`) *
      (isNegative ? -1 : 1)
    );
  }
  ...
  for (const word of words.split(/[\s-]+/)) {
    if (word === "and") continue; // Ignore "and" (e.g., "one hundred and two")
